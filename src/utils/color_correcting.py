...
def _normalizing_interval(array):
    high = 255
    low = 0
    max_dist = 0
...
def _get_filter_matrix(mat: np.ndarray) -> np.ndarray:
    ...
    while new_avg_r < MIN_AVG_RED:
        shifted = _hue_shift_red(avg_mat, hue_shift)
    ...
    shifted_mat = _hue_shift_red(mat, hue_shift)
    ...
    adjust_r_low, adjust_r_high = _normalizing_interval(normalize_mat[..., 0])
    adjust_g_low, adjust_g_high = _normalizing_interval(normalize_mat[..., 1])
    adjust_b_low, adjust_b_high = _normalizing_interval(normalize_mat[..., 2])

    shifted = _hue_shift_red(np.array([1, 1, 1]), hue_shift)
